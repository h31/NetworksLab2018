...
void internal_log(char const* caller_func, char* msg)
{
    ...
    char pthread_id_buf[10 + 6]; // Buffer for thread id, 10 = max int size, 6= "thr:" + ": "
    ...
    output = (char*)malloc((strlen(msg) + strlen(caller_func) + 32 + strlen(pthread_id_buf)) * sizeof(char));
